    ...
    public partial class MB3_MeshCombinerSingle : MB3_MeshCombiner
    {
        ...
        */
            ...
            for (int i = 0; i < toAddDGOs.Count; i++)
            {
                ...
                { //for skinned meshes leave in bind pose
                    for (int j = 0; j < nverts.Length; j++)
                    {
                        ...
                        if (settings.doNorm)
                        {
                            normals[vIdx] = go.transform.TransformDirection(nnorms[j]);
                            normals[vIdx] = normals[vIdx].normalized;
                        }
                        if (settings.doTan)
                        {
                            float w = ntangs[j].w; //need to preserve the w value
                            Vector3 tn = go.transform.TransformDirection(ntangs[j]);
                            tn.Normalize();
                            tangents[vIdx] = tn;
                            tangents[vIdx].w = w;
                        }
        ...
        public override void Apply(GenerateUV2Delegate uv2GenerationMethod)
        {
            ...
            Apply(true, true, settings.doNorm, settings.doTan, settings.doUV, doUV2(), settings.doUV3, settings.doUV4, settings.doCol, doBones, settings.doBlendShapes, uv2GenerationMethod);
        }
                          ...
                          GenerateUV2Delegate uv2GenerationMethod = null)
        {
            ...
            if (_mesh != null)
            {
                ...
                if (vertices)
                {
                    ...
                    if (verts.Length > 0) {
                        if (settings.recenterVertsToBoundsCenter && settings.renderType == MB_RenderType.meshRenderer)
                        {
                ...
                if (colors)
                {
                    if (settings.doCol) { _mesh.colors = this.colors; }
